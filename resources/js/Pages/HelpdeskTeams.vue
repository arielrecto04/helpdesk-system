...
const dataTeams = computed(() => {
    return props.teams && props.teams.data ? props.teams.data : (Array.isArray(props.teams) ? props.teams : []);
});
                    ...
                    <div class="flex justify-between items-center">
                        <div class="flex items-center">
                            ...
                            <div>
                                ...
                                <p class="text-blue-100 text-sm mt-1">Overview of all helpdesk teams</p>
                            </div>
                        </div>
                        <Link v-if="userPermissions && userPermissions.includes('create_helpdeskteams')" :href="route('helpdeskteams.create')" class="inline-flex items-center px-6 py-3 bg-white text-indigo-600 rounded-xl font-semibold text-sm shadow-lg hover:bg-blue-50 hover:shadow-xl transform hover:scale-105 transition-all duration-300">
            ...
            <div class="max-w-screen-2xl mx-auto px-4 sm:px-6 lg:px-8">
                <div class="bg-white overflow-hidden shadow-xl rounded-2xl border border-gray-400">
                    <div class="p-6 text-gray-900">
                        <div class="overflow-x-auto">
                            <table class="min-w-full divide-y divide-gray-200">
                                <thead class="bg-gradient-to-r from-gray-300 to-gray-200">
                                    <tr>
                                        <th scope="col" class="px-6 py-4 text-left text-xs font-bold text-gray-900 uppercase tracking-wider">ID</th>
                                        <th scope="col" class="px-6 py-4 text-left text-xs font-bold text-gray-900 uppercase tracking-wider">Team Name</th>
                                        <th scope="col" class="px-6 py-4 text-left text-xs font-bold text-gray-900 uppercase tracking-wider">Members</th>

                                    </tr>
                                </thead>
                                <tbody class="bg-white divide-y divide-gray-300">
                                    <tr v-if="dataTeams.length === 0">
                                        <td colspan="5" class="px-6 py-16 text-center">
                                            <div class="flex flex-col items-center">
                                                ...
                                                <p class="text-lg font-semibold text-gray-600 mb-2">No Teams Found</p>
                                                <p class="text-sm text-gray-500">There are no helpdesk teams to display.</p>
                                            </div>
                                        </td>
                                    </tr>
                                    <tr v-for="team in dataTeams" :key="team.id" @click="viewTeam(team.id)" 
                                    class="group hover:bg-transparent cursor-pointer transition-all duration-200">
                                        ...
                                        <td class="px-6 py-4 whitespace-nowrap group-hover:bg-gradient-to-r group-hover:from-blue-50 group-hover:to-indigo-50">
                                            <div class="flex items-center">
                                                <span class="text-sm text-gray-700 font-medium">{{ team.members_count ?? (team.members ? team.members.length : 'N/A') }}</span>
                                            </div>
                                        </td>
                                    </tr>
                                </tbody>
                        ...
                        <div v-if="props.teams && props.teams.links && props.teams.links.length > 3" class="mt-6">
                           <Pagination :links="props.teams.links" />
                        </div>
                    </div>
                </div>
            </div>
