<template>
    <ShowLayout
        :title="helpdesk_team.team_name"
        subtitle="Helpdesk Team Information"
        icon="users"
        :actions="userPermissions && (userPermissions.includes('edit_helpdeskteams') || userPermissions.includes('delete_helpdeskteams'))"
    >
        <template #actions>
            <Link v-if="userPermissions.includes('edit_helpdeskteams')" :href="route('helpdeskteams.edit', helpdesk_team.id)" class="inline-flex items-center px-6 py-3 bg-white text-indigo-600 rounded-xl font-semibold text-sm shadow-lg hover:bg-blue-50 hover:shadow-xl transform hover:scale-105 transition-all duration-300">
            ...
            <DangerButton v-if="userPermissions.includes('delete_helpdeskteams')" @click="confirmTeamDeletion" class="bg-white/20 text-white hover:bg-white/30 rounded-xl px-6 py-3">
                Delete
            </DangerButton>
        </template>
        ...
        <div class="bg-white overflow-hidden shadow-xl rounded-2xl border border-gray-400">
            <div class="p-6 text-gray-900">
                <div class="flex items-center mb-6 pb-4 border-b">
                    ...
                    <div>
                        <h2 class="font-bold text-xl text-gray-800">Team Details</h2>
                        <p class="text-sm text-gray-500">ID: #{{ helpdesk_team.id }}</p>
                    </div>
                </div>
                    ...
                    <div>
                        ...
                        <dl class="grid grid-cols-1 gap-4">
                            ...
                            <div>
                                ...
                                <dd class="mt-1 text-sm text-gray-900">{{ helpdesk_team.team_name }}</dd>
                            </div>
                        </dl>
                    </div>
                </div>
