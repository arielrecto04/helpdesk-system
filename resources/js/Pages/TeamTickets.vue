...
const getPersistedState = (key, defaultValue) => {
    const stored = localStorage.getItem(`teamTicketsFilters_${key}`);
...
const toggleFilters = () => {
    ...
    localStorage.setItem('teamTicketsFilters_showFilters', JSON.stringify(showFilters.value));
};

// Persist filter changes to localStorage
watch([search, stageFilter, priorityFilter, assignedFilter, dateFrom, dateTo, deadlineFrom, deadlineTo, sortField, sortDirection], () => {
    localStorage.setItem('teamTicketsFilters_search', JSON.stringify(search.value));
    localStorage.setItem('teamTicketsFilters_stage', JSON.stringify(stageFilter.value));
    localStorage.setItem('teamTicketsFilters_priority', JSON.stringify(priorityFilter.value));
    localStorage.setItem('teamTicketsFilters_assigned', JSON.stringify(assignedFilter.value));
    localStorage.setItem('teamTicketsFilters_dateFrom', JSON.stringify(dateFrom.value));
    localStorage.setItem('teamTicketsFilters_dateTo', JSON.stringify(dateTo.value));
    localStorage.setItem('teamTicketsFilters_deadlineFrom', JSON.stringify(deadlineFrom.value));
    localStorage.setItem('teamTicketsFilters_deadlineTo', JSON.stringify(deadlineTo.value));
    localStorage.setItem('teamTicketsFilters_sortField', JSON.stringify(sortField.value));
    localStorage.setItem('teamTicketsFilters_sortDirection', JSON.stringify(sortDirection.value));
});
...
const applyFilters = debounce(() => {
    router.get(route('teamtickets.index'), {
        search: search.value,
        ...
        // Ensure requests stay scoped to the current team (controller expects `team`)
        team: props.team && props.team.id ? props.team.id : null,
...
watch([search, stageFilter, priorityFilter, assignedFilter, dateFrom, dateTo, deadlineFrom, deadlineTo], () => {
    applyFilters();
});
...
watch(() => props.tickets.data, () => {
    ...
    selectAll.value = selectedTickets.value.length === filteredTickets.value.length && filteredTickets.value.length > 0;
});
...
const executeBulkAction = () => {
    if (bulkAction.value === 'delete') {
        ...
        router.post(route('teamtickets.bulk-delete'), {
        ...
        router.post(route('teamtickets.bulk-update'), {
            ticket_ids: selectedTickets.value,
            updates: updates
        }, {
            onSuccess: () => {
                        ...
                        <div class="mb-5 flex items-center justify-between">
                            <div class="flex items-center gap-2">
                                <span class="bg-indigo-100 text-indigo-800 text-xs font-semibold px-2.5 py-0.5 rounded border border-indigo-200">
                                    Showing: {{ tickets.from || (filteredTickets.length ? 1 : 0) }}–{{ tickets.to || filteredTickets.length }} of {{ tickets.total }}
                                </span>
                        ...
                        <div class="overflow-x-auto">
                            <table class="min-w-full divide-y divide-gray-200">
                                ...
                                <tbody class="bg-white divide-y divide-gray-300">
                                    ...
                                    <tr v-for="ticket in filteredTickets" :key="ticket.id" class="group hover:bg-transparent transition-all duration-200">
                                        ...
                                        <td @click="viewTeamTicket(ticket.id)" class="px-6 py-4 whitespace-nowrap border-l-4 border-transparent group-hover:border-indigo-500 group-hover:bg-gradient-to-r group-hover:from-blue-50 group-hover:to-indigo-50 cursor-pointer">
                                            <div class="flex items-center">
                                        ...
                                        <td @click="viewTeamTicket(ticket.id)" class="px-6 py-4 group-hover:bg-gradient-to-r group-hover:from-blue-50 group-hover:to-indigo-50 cursor-pointer">
                                            <div class="flex items-center">
