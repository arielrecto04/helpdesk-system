...
<template>
    ...
    <AuthenticatedLayout>
        <div class="flex justify-between items-center">
            ...
            <Link v-if="userPermissions && userPermissions.includes('create_employees')" :href="route('employees.create')" class="inline-flex items-center px-4 py-2 bg-gray-800 border border-transparent rounded-md font-semibold text-xs text-white uppercase tracking-widest hover:bg-gray-700 focus:bg-gray-700 active:bg-gray-900 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:ring-offset-2 transition ease-in-out duration-150">
                Create Employee
            </Link>
        </div>
        <div class="py-12">
                            ...
                            <table class="min-w-full divide-y divide-gray-200">
                                ...
                                <tbody class="bg-white divide-y divide-gray-200">
                                    ...
                                    <tr v-for="employee in employees.data" :key="employee.id" @click="viewEmployee(employee.id)" class="hover:bg-gray-100 cursor-pointer">
                                        ...
                                        <td class="px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-900">{{ employee.name ?? (employee.first_name + ' ' + employee.last_name) }}</td>
                                        <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">{{ employee.email }}</td>
                                        <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">{{ employee.company_name ?? 'N/A' }}</td>
